...
<html>
  ...
  <body>
    <div class="box_2">
	    <div class="div_title"><h4>修改奖项</h4></div>
	    <input id="id" type="hidden" value="<%=data.get("id") %>"/>
	    <table id="table_input_2">
			<tr>
			    <td colspan="2">奖项名称:<span id="msg_" class='msg_common'></span><br />
	        ...
	        <tr valign="top">
	            <td>
			        <table>
			            ...
			            <tr>
			                <td>级别:<span id="msg_" class='msg_common'></span><br />
			                    <input id="level" value="<%=data.get("level") %>" type="text" />
			                </td>
			            </tr>
			            <tr>
			                <td>获奖日期<span class='msg_format'>(格式:yyyy-mm)</span>:<span id="msg_" class='msg_common'></span><br />
			                    <input readonly="readonly" value="<%=data.get("date") %>" class="date" id="date" type="text" />
			                </td>
			            </tr>
  		                <tr class="">
  		                    <td>2013类型:<br />
	            ...
	            <td>
			        <table>
			            <tr>
			                <td>授奖部门:<span id="msg_" class='msg_common'></span><br />
			                    <input id="department" value="<%=data.get("department") %>" type="text" />
			                </td>
			            </tr>
			            <tr class="tr_type tr_type">
			                <td>类型:<br />
			                    ...
			                    <% for (Map<String, String> type: typeList1) { %>
			                        <option title="<%=type.get("a_value") %>" value='<%=type.get("id") %>' <% if (type.get("id").equals(data.get("type_id"))) { %>selected<% } %>>
			                            <%=type.get("name") %>
			                        </option>
			                    <%} %>
			                    </select>
	    ...
	    <div class="box_4">
	    	<h4>说明</h4>
	    	<p><%=(String)application.getAttribute("submitTipAwards") %></p>
	    </div>
    </div>
  ...
  function modify() {
	     ...
	     $(".tr_member").each(function (){
	         ...
	         if ($(this).find(".member_name").val() == "") {
	        	 $(this).find(".msg_common").text("请填写获奖人！");
	             flag = false;
	     ...
	     if (date == "") {
	         $("#date").parent().find(".msg_common").text("不能为空！");
	         flag = false;    
	     } else if (date > dateE || date < dateS) {
	            $("#date").parent().find(".msg_common").text("只能为当年成果！");
	            flag = false;    
	        }
	     if (level == "") {
	         $("#level").parent().find(".msg_common").text("不能为空！");
	         flag = false;    
	     }
	     if (department == "") {
	         $("#department").parent().find(".msg_common").text("不能为空！");
	         flag = false;    
	     }
	     if (file_path == "") {
	  ...
	  $.ajax({
			...
			success: 
			    function(result){
		            if (result == '1') {	
			            alert("修改成功！");
		                window.location.href = "awardsList.do?q=0&f=1";
		            } else if (result == '0') {
			            alert("修改失败！");
				    } else {
			            alert("修改失败！"+result);
